public class AddressablesNames
{
    // ‰æ‘œŠÖ˜A
    public const string BACK_SPRITE = "Assets/Images/back.png";
    public const string FRONT_ACE = "Assets/Images/ace.png";
    public const string FRONT_TWO = "Assets/Images/two.png";
    public const string FRONT_THREE = "Assets/Images/three.png";
    public const string FRONT_FOUR = "Assets/Images/four.png";
    public const string FRONT_FIVE = "Assets/Images/five.png";
    public const string FRONT_SIX = "Assets/Images/six.png";
    public const string FRONT_SEVEN = "Assets/Images/seven.png";
    public const string FRONT_EIGHT = "Assets/Images/eight.png";
    public const string FRONT_NINE = "Assets/Images/nine.png";
    public const string FRONT_TEN = "Assets/Images/ten.png";
    public const string FRONT_JACK = "Assets/Images/jack.png";
    public const string FRONT_QUEEN = "Assets/Images/queen.png";
    public const string FRONT_KING = "Assets/Images/king.png";


    // prefabŠÖ˜A
    public const string UI_HP_TEXT = "Assets/Prefabs/HP.prefab";
    public const string UI_START_COUNT_TEXT = "Assets/Prefabs/StartCountText.prefab";
    public const string GOAL_LINE = "Assets/Prefabs/goal_line.prefab";
    public const string PLAYABLE_CHARACTER = "Assets/Prefabs/playableIcon.prefab";
    public const string TARGET_ICON_LIST = "Assets/Prefabs/target_icon.prefab";
}
